...
| 파일 | 설명 | 기간 |
|------|------|------|
| train.csv | 100개 건물 전력소비량 + 기상 데이터 | 2024.06.01 ~ 08.24 |
| test.csv | 기상 예보 데이터 (일조/일사 없음) | 2024.08.25 ~ 08.31 |
| building_info.csv | 건물 유형, 면적, 태양광/ESS 용량 | - |
...
test : 2024.08.25 ~ 08.31 (7일)    16,800행
...
> ⚠️ building_hour_mean 등 통계 feature는
> 반드시 **train 데이터에서만** 계산 후 test에 적용  
> test의 미래값을 미리 보면 실제 서비스에서 사용 불가능한 모델이 됨
...
| 실험 2 | LightGBM으로 교체 | 9.39 | -0.96 |
| 실험 3 | 건물 유형별 모델 분리 (XGBoost) | 7.26 | +1.13 |
...
**3. test 데이터의 일조/일사 부재**

train에는 있지만 test에는 없는 feature로
예측 시 정보 손실 발생
